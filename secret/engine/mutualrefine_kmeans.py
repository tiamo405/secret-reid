...
class mutualrefine(object):
    ...
    def __init__(self, cfg, n_clusters=700):
        self.cfg = cfg
    ...
    def _build_dataset(self):
        ...
        print(self.Target_cluster_loader.dataset[0])
        self.Target_test_loader = build_loader(self.cfg, self.Target_dataset, is_train=False)
        self.num_classes = min(len(self.Target_dataset.train), self.n_clusters)
    ...
    def generate_pseudo_dataset(self, epoch):
        ...
        self.labels_list = []
        kms = [
            MiniBatchKMeans(n_clusters=self.n_clusters, max_iter=100, batch_size=300, init_size=2500),
            MiniBatchKMeans(n_clusters=self.n_clusters, max_iter=100, batch_size=300, init_size=1500),
            MiniBatchKMeans(n_clusters=self.n_clusters, max_iter=100, batch_size=300, init_size=1500)
        ]
        ...
        for i in range(part_num):
            ...
            km = kms[i].fit(cf[i])
            # km = KMeans(n_clusters=n_clusters, random_state=1, max_iter=300).fit(cf[i])
            self.labels_list.append(km.labels_)
            # np.random.seed(0)
            # self.labels_list.append(np.random.randint(0, self.n_clusters, (cf[i].shape[0],)))
            self.num_clusters_list.append(self.n_clusters)
            self.logger.info('Clustered into {} classes'.format(self.n_clusters))
        ...
        for i, (fname, _, cid) in enumerate(sorted(self.Target_dataset.train)):
            ...
            fnames = pseudo_label.get(label[0], [])
            fnames.append([fname, cid])
            pseudo_label[label[0]] = fnames

        with open(f'{self.cfg.OUTPUT_DIR}/pseudo_label.json', 'w') as convert_file:
            convert_file.write(json.dumps(pseudo_label, indent=4))

        # viz pseudo_label
        img_shape = (256, 128)  # (h,w)
        row_col = (50, 5)
        max_row = 200
        images = []
        pseudo_label = list(pseudo_label.values())
        random.shuffle(pseudo_label)
        for paths in pseudo_label[:]:
            row_image = np.zeros((img_shape[0], img_shape[1]*row_col[1], 3), dtype=np.uint8)
            for i, p in enumerate(paths[:row_col[1]]):
                fname, cid = p
                p_img = cv2.imread(fname)
                p_img = cv2.resize(p_img, img_shape[::-1])
                cv2.putText(p_img, f'{cid:.2f}', (50,100), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0,255,0), 2, cv2.LINE_AA)
                row_image[:, i*img_shape[1]:(i+1)*img_shape[1], :] = p_img.copy()
            images.append(row_image)
        print("write cluster data...")
        for i in range(0, len(images), max_row):
            image = np.concatenate(images[i:i+max_row], axis=0)
            cv2.imwrite(f'{self.cfg.OUTPUT_DIR}/pseudo_label_{i}.jpg', image)
        del dict_f, cf
        torch.cuda.empty_cache()

        self.logger.info('new dataset length: {}, total id: {}'.format(len(new_dataset), len(pseudo_label)))
    ...
    def eval_save(self, epoch):
        ...
        if len(self.Target_dataset.query):
